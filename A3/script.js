/* 
  ...
  To solve this, we implement a decoupled scrolling loop:
  ...
  - Scroll Depth as a Timeline: By mapping vertical scroll to horizontal 
    translation ('transform: translateX()'), we give the user tactile, 
  ...
  - Active State Mapping: We use bounding box coordinates to detect which 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const lecturesTextScrollable = document.getElementById('lectures-text-scrollable');
  const lecturesTextContainer = document.getElementById('lectures-text-container');
  ...
  function updateLayout() {
    ...
    if (act2 && floatingCyclone) {
      ...
      if (rect2.top < window.innerHeight && rect2.bottom > 0) {
        ...
        const progressAct2 = (window.innerHeight - rect2.top) / (act2Height + window.innerHeight);
        
        // Translate and spin the vector cyclone (increased speed slightly)
    ...
    if (galleryTop <= 0 && galleryTop > -totalScrollableGallery) {
      ...
      if (lecturesTextScrollable && lecturesTextContainer) {
        const maxTextTranslate = Math.max(0, lecturesTextScrollable.scrollHeight - lecturesTextContainer.clientHeight + 40); // 40px padding buffer
      ...
      const galleryViewport = document.querySelector('.lectures-gallery-column');
      const galleryViewportWidth = galleryViewport ? galleryViewport.clientWidth : window.innerWidth * 0.6;
  ...
  // ----------------------------------------------------
  // Interactive Outro & Lightbox Modal System
  // ----------------------------------------------------
  
  /**
   * Opens lightbox modal and populates it with the target slide image and data
   */
  ...
  function parseFilename(filename) {
    ...
    // Remove trailing codes like -u46 or -g7
    ...
    if (pdrNum === '1033304') {
      meta = "PDR Slide #1033304 | Inscription: 'Great ruler of an immortal cosmos, give me truth.'";
    } else if (pdrNum === '1033200') {
      meta = "PDR Slide #1033200 | Inscription: 'Saturn from satellite local.' Hand-painted celestial perspective.";
    } else if (pdrNum === '1033421') {
      meta = "PDR Slide #1033421 | Detail of the lunar surface craters and mountain ridges.";
    } else if (pdrNum === '1033190') {
      meta = "PDR Slide #1033190 | Inscription: 'Dust shoals from the sun bombarding the earth.'";
    } else if (pdrNum === '1033329') {
      meta = "PDR Slide #1033329 | Rare photograph capturing storm electricity in Queensland, Australia.";
    } else if (pdrNum === '1033185') {
      meta = "PDR Slide #1033185 | Corona of the Sun during a total solar eclipse, late 19th century.";
    } else if (pdrNum === '1033327') {
      meta = "PDR Slide #1033327 | Inscription: 'How radium writes its autograph.' Illustrating radiant particles.";
    } else if (pdrNum === '1033446') {
      meta = "PDR Slide #1033446 | Clement Lindley Wragge standing in his tropical garden haven in Auckland.";
    } else if (pdrNum === '1033287') {
      meta = "PDR Slide #1033287 | Magic lantern lecture title slide demonstrating theosophical concepts.";
    } else if (pdrNum === '1033381') {
      meta = "PDR Slide #1033381 | Inscription expressing the fundamental motto of theosophy.";
    }
  ...
  /**
   * Opens the lightbox modal for any arbitrary slide in the 300+ image archive
   */
